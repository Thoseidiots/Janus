...
class JanusDataset(Dataset):
    ...
    def __init__(self, tokenizer: AvusTokenizer, block_size: int,
        ...
        # __file__ is undefined when the script is run via exec() in a Kaggle
        # notebook cell. Fall back to cwd so path resolution still works.
        try:
        ...
        # ── Human-like language dataset ──────────────────────────────────────
        # 12 dimensions: opinions, emotions, casual speech, personal stories,
        # curiosity, humor, uncertainty, empathy, debate, aspirations,
        # self-reflection, and identity/self-awareness.
        # Generated by human_dataset/generate_human_dataset.py.
        _human_candidates = [
            _script_dir / "human_dataset" / "output" / "avus_human_pairs.txt",
            Path("/kaggle/input/janus-avus-weights/human_dataset/output/avus_human_pairs.txt"),
            Path("/kaggle/working/human_dataset/output/avus_human_pairs.txt"),
            Path("human_dataset/output/avus_human_pairs.txt"),
        ]
        for _human_path in _human_candidates:
            if _human_path.exists():
                _human_lines = _human_path.read_text(encoding="utf-8").splitlines()
                _human_lines = [l.strip() for l in _human_lines if l.strip()]
        ...
        # product questions, supplier queries, anomaly detection.
        # Generated by synthetic_database/generate_rich_training.py.
        # Searches several known paths so it works both locally and on Kaggle.
        _synth_candidates = [
            _script_dir / "synthetic_database" / "output" / "avus_training_pairs.txt",
            Path("/kaggle/input/janus-avus-weights/synthetic_database/output/avus_training_pairs.txt"),
            Path("/kaggle/working/synthetic_database/output/avus_training_pairs.txt"),
            Path("synthetic_database/output/avus_training_pairs.txt"),
        ]
        for _synth_path in _synth_candidates:
            if _synth_path.exists():
                _synth_lines = _synth_path.read_text(encoding="utf-8").splitlines()
                _synth_lines = [l.strip() for l in _synth_lines if l.strip()]
