...
janus_gpt.py
============
GPT-2-style decoder-only transformer matching the janus-v1 training architecture.
...
Falls back to a simple UTF-8 byte tokenizer if tiktoken is unavailable.
"""
...
class JanusConfig:
    ...
    def from_summary(cls, path: str) -> "JanusConfig":
        """Load config from a janus_training_summary.json file."""
...
class JanusTokenizer:
    """
    Wraps tiktoken (GPT-2 encoding) to match the training tokenizer.
    Falls back to UTF-8 byte encoding if tiktoken is not installed.
    """
...
class JanusGPT(nn.Module):
    ...
    def from_checkpoint(
        ...
        """Load a trained checkpoint, reading config from training summary if provided."""
        config = JanusConfig()
        ...
        model.eval()
        print(
            f"[JanusGPT] Loaded from {weights_path} "
    ...
    def generate(
        ...
        self.eval()
        ids = self._tokenizer.encode(prompt) or [0]
    ...
    def get_embedding(self, text: str) -> torch.Tensor:
        """
        Returns a mean-pooled embedding vector for text (shape: n_embd = 384).
        Used by HomeostasisEngine as a stimulus input.
        """
        ids = self._tokenizer.encode(text)[: self.config.block_size] or [0]
        idx = torch.tensor([ids], dtype=torch.long)
        with torch.no_grad():
...
def load_janus_brain(
    ...
    """
    Load janus_best.pt (or janus_final.pt) from the weights directory,
    reading config from janus_training_summary.json if present.
    ...
    if not os.path.exists(weights):
        raise FileNotFoundError(
            ...
            "Ensure my-llm-project/weights/ exists and contains the checkpoint."
        )
