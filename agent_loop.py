...
agent_loop.py
=============
Janus Agent Loop — ties goal_planner, tool_executor, web_autonomy,
memory_manager, and core into a single coherent execution loop.

Takes a high-level goal and executes it end-to-end.
    ...
    def handle(self, tool: str, args: dict) -> Any:
        if tool == "web_navigate":
            ...
            try:
                ...
                results  = [el.inner_text()[:500] for el in elements[:limit]]
                return results
            except Exception as e:
                return {"error": str(e)}
...
class AgentMemory:
    ...
    def recent_plans(self, n: int = 10) -> list:
...
class JanusAgent:
    ...
    def run_goal(self, goal: str, context: dict = {}) -> AgentPlan:
        ...
        plan.outcome   = f"{successes}/{len(plan.steps)} steps completed successfully"

        self._log(f"Plan complete: {plan.outcome}")

        if self._core:
            self._core.perceive(f"Goal '{goal}' result: {plan.outcome}")
    ...
    def run_continuous(self, goals: list, delay: float = 5.0):
