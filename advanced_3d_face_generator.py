...
class ProceduralFaceGenerator:
    ...
    def _create_base_topology(self) -> Dict:
        """Creates anatomically-aware base face mesh topology"""
        vertices = []
        faces = []
        uv_coords = []
        ...
        for ring in range(rings + 1):
            # Non-linear ring distribution for better face proportions
            ring_factor = ring / rings
            
            # Adjust theta for more realistic head shape
            if ring_factor < 0.3:  # Forehead region
                theta = ring_factor * 0.6 * np.pi
            elif ring_factor < 0.7:  # Face region
                theta = 0.6 * np.pi + (ring_factor - 0.3) * 0.8 * np.pi
            else:  # Chin/neck region
                theta = 0.6 * np.pi + 0.8 * 0.4 * np.pi + (ring_factor - 0.7) * 0.6 * np.pi
            ...
            for segment in range(segments):
                ...
                if abs(phi - np.pi) < np.pi/3:  # Back region
                    z *= 0.7
                
                # Create eye socket indentations
                if 0.3 < ring_factor < 0.6 and (abs(phi - np.pi/4) < np.pi/8 or abs(phi - 7*np.pi/4) < np.pi/8):
                    z *= 0.85
                
                # Nose protrusion
                if 0.4 < ring_factor < 0.65 and abs(phi) < np.pi/12:
                    z *= 1.3
                
                # Mouth indentation
                if 0.65 < ring_factor < 0.8 and abs(phi) < np.pi/6:
                    z *= 0.95
                ...
                # Assign to anatomical groups
                vertex_idx = len(vertices) - 1
                self._assign_vertex_group(vertex_idx, ring_factor, phi, vertex_groups)
        ...
        for ring in range(rings):
            for segment in range(segments):
                ...
                # Create quads split into triangles
    ...
    def _assign_vertex_group(self, vertex_idx: int, ring_factor: float, phi: float, groups: Dict):
        """Assign vertex to anatomical groups for targeted deformation"""
        if 'forehead' not in groups:
            groups.update({
        ...
        if ring_factor < 0.25:
            groups['forehead'].append(vertex_idx)
        elif 0.25 <= ring_factor < 0.45:
            if abs(phi - np.pi/4) < np.pi/6 or abs(phi - 7*np.pi/4) < np.pi/6:
                groups['eyes'].append(vertex_idx)
            else:
                groups['scalp'].append(vertex_idx)
        elif 0.45 <= ring_factor < 0.65:
            if abs(phi) < np.pi/8:
                groups['nose'].append(vertex_idx)
            elif abs(phi - np.pi/2) < np.pi/4 or abs(phi - 3*np.pi/2) < np.pi/4:
                groups['cheeks'].append(vertex_idx)
            elif abs(phi - np.pi/2) < np.pi/6 or abs(phi - 3*np.pi/2) < np.pi/6:
                groups['ears'].append(vertex_idx)
        elif 0.65 <= ring_factor < 0.8:
            if abs(phi) < np.pi/6:
                groups['mouth'].append(vertex_idx)
            else:
                groups['cheeks'].append(vertex_idx)
        else:
            groups['jaw'].append(vertex_idx)
    ...
    def _generate_procedural_texture(self, features: FacialFeatures, resolution: int = 512) -> np.ndarray:
        ...
        if features.age > 0.4:
            ...
            texture[wrinkle_mask] = (texture[wrinkle_mask] * 0.8).astype(np.uint8)
