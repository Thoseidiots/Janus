...
subprocess sandbox. Every call is appended to an append-only event trace.
"""
...
class ToolExecutor:
    ...
    def _verify(self, call: ToolCall) -> Tuple[bool, str]:
        ...
        for param, ptype in spec.parameters.items():
            if param not in call.args and ptype == "str":
    ...
    def _apply_sandboxed(self, call: ToolCall) -> Tuple[Any, Optional[str]]:
        if call.tool == "code_exec":
            ...
            return {"status": 200,
                    "body": f"[offline stub for {call.args.get('url', '')}]"}, None
    ...
    def _tool_file_write(self, args: dict) -> Any:
        p = Path(args.get("path", "janus_out.txt"))
    ...
    def _tool_shell_cmd(self, args: dict) -> Any:
        ALLOWED = {"ls", "dir", "pwd", "echo", "date", "whoami", "uname"}
        cmd  = args.get("cmd", "")
